...
public interface ReadingQueryService {
    ...
    ReadingWorkDetailVO detail(Long workId, Long sourceId, String refreshPolicy);

    ReadingPageVO<ReadingSourceVO> sources(Long workId, int page, int pageSize);

    ReadingPageVO<ReadingSourceVO> sourceDefinitions(int page, int pageSize);

    List<ReadingFilterVO> categories(int limit);

    List<ReadingFilterVO> filters(String type, int limit);
